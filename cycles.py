...
def get_centroid(bps):
    #input nx2 array of boundary points
    xc=np.average(bps[:,0])
    yc=np.average(bps[:,1])
    return xc,yc
...
def get_vol_opt_cycle_centroid_coords(persist,pd1):
    bps=[]
    cycle_xcs=np.zeros((len(persist)))
    cycle_ycs=np.zeros((len(persist)))
    for i in range(0,len(persist)):
        pair=hc.Pair(pd1,i)
        opt_vol=pair.optimal_volume()
        bps.append(np.asarray(opt_vol.boundary_points()))
        cycle_xcs[i],cycle_ycs[i]=get_centroid(bps[i])
    return cycle_xcs,cycle_ycs,bps
...
def get_left_p_wave_coords(idx_p,opt_vol_bps,r_peak_xcs): #this will return the left-hand endpoint of the right-most detected p-wave (if one exists) in each RR-interval
    left_raw=[]
    left=[]
...
def get_all_right_s_wave_coords(right_s_wave_coords,r_peak_xcs,ekg):#finish this function to make it return the first x-coordinate of the first datum from ekg above the baseline after the dip from the r-wave
